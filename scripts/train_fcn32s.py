...
使用 models.py 中的 vgg16 + fcn32 构建 FCN-32s 语义分割模型，
从 TFRecord 读取 21 类 VOC 数据进行训练与验证。

输出:
    - checkpoint 模型: checkpoints/fcn32s_21class/
    - 训练日志 (控制台输出): loss, pixel accuracy, mean IoU

Usage:
    # Smoke test (CPU 可运行)
    python scripts/train_fcn32s.py --epochs 1 --batch-size 1 --image-size 224

    # 完整训练
    python scripts/train_fcn32s.py --epochs 50 --batch-size 8 --image-size 320 --lr 1e-4
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="FCN-32s 语义分割训练脚本 (21 类)"
    )
    # 数据
    parser.add_argument(
        "--train-record",
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="checkpoints/fcn32s_21class",
        help="模型保存目录，默认 checkpoints/fcn32s_21class",
    )
...
def build_fcn32s_model(image_size, num_classes=21, l2=0.0, dropout=0.0):
    """构建 FCN-32s 模型: vgg16 → fcn32。
    ...
    Returns:
        (keras Model) FCN-32s 模型。
    """
    from models import vgg16, fcn32

    vgg16_model = vgg16(l2=l2, dropout=dropout)
    model = fcn32(vgg16_model, l2=l2)
    return model
